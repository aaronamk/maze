 ...
 * Test case: Test the size of the maze.
	...
	/**
	 * Test case: The path to the exit works properly
	 * <p>
	 * Correct behavior: Each square has a square closer to the exit next to it.
	 */
	@Test
	public void testNeighborCloserToExitIsCloser() {
		for(int i=0; i<24; i++) {
			for(int j=0; j<24; j++) {
				assertTrue(MazeConfig.getDistanceToExit(MazeConfig.getNeighborCloserToExit(i, j)[0], MazeConfig.getNeighborCloserToExit(i, j)[1]) < MazeConfig.getDistanceToExit(i, j));
			}
		}
	}
	...
	public void testDeterminism() {
		StubOrder NewOrder = new StubOrder(5, Builder.Prim, true);
